...
def _browser_only_html_guard(template: dict[str, Any], values: dict[str, Any], output_path):
    if template.get("template_engine") == "html" and template.get("html_template"):
        raise RuntimeError("HTML templates print from the user's local browser in Ziad Invoices 3.3.42")
...
def _browser_printing_status() -> dict[str, Any]:
    return {
        ...
        "playwright": False,
        "message": "Payment Request user-entered data prints at a physical 16pt; other HTML document data remains 15pt on native A4 sizing",
    }
...
async def runtime_cache_headers(request: core.Request, call_next):
    ...
    response.headers["X-Ziad-Print-Engine"] = "browser-local-native-a4-pr16pt-3.3.42"
    return response
