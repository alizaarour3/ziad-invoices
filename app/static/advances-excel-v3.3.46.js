...
(() => {
  if (window.__ziadAdvancesExcel3346) return;
  window.__ziadAdvancesExcel3346 = true;
  ...
  async function fetchDownloadBlob(path) {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      try {
        ...
        if (!blob || !blob.size) throw new Error('ملف Excel المستلم فارغ');
        return blob;
      } catch (error) {
        lastError = error;
    ...
    throw lastError || new Error('تعذر تنزيل ملف Excel');
  }
  ...
  async function downloadApi(path, filename, button) {
    ...
    try {
      ...
      const message = error?.message === 'Failed to fetch'
        ? 'انقطع الاتصال أثناء تنزيل Excel. أعد المحاولة بعد ثوانٍ.'
        : (error?.message || 'تعذر تصدير ملف Excel');
      toast(message, 'error');
    } finally {
  ...
  async function importExcel(file, input, button) {
    ...
    try {
      ...
      const message = error?.message === 'Failed to fetch'
        ? 'انقطع الاتصال أثناء رفع ملف Excel. لم تتم إعادة الإرسال تلقائياً لمنع التكرار؛ أعد المحاولة مرة واحدة.'
        : (error?.message || 'تعذر استيراد ملف Excel');
      toast(message, 'error');
    } finally {
      input.value = '';
      if (button?.isConnected) {
